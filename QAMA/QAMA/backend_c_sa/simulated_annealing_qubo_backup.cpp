...
torch::Tensor simulated_annealing_qubo(const torch::Tensor& Q,
    ...
    while ((temperatures > cutoff_temperature).any().item<bool>() && no_improvement_count < patience) {
        for (int i = 0; i < iterations_per_t; i++) {
            ...
            // //debug check is delta_energies is correct
            // torch::Tensor delta_old_E = (current_solutions.unsqueeze(2) * current_solutions.unsqueeze(1) * Q).sum({1, 2});
            // torch::Tensor delta_new_E = (new_solutions.unsqueeze(2) * new_solutions.unsqueeze(1) * Q).sum({1, 2});
            // torch::Tensor delta_energies_check = delta_new_E - delta_old_E;
            // std::cout<< "Delta energies check: " << delta_energies_check << std::endl;
            // std::cout<< "Delta energies: " << delta_energies << std::endl;
            // TORCH_CHECK(torch::allclose(delta_energies, delta_energies_check, 1e-3, 1e-3),
            //             "Delta energies calculation is incorrect.");
        ...
        // //debug check is energy decrease
        // torch::Tensor test = (best_solutions.unsqueeze(2) * best_solutions.unsqueeze(1) * Q).sum({1, 2});
        // std::cout << "Current energies: " << test << std::endl;
    }
